...
#### ExecutorServiceInstance.invokeAny
参数为集合，储存的是callable接口。也就是一系列任务组tasks，返回类型是callable的泛型，值为该系列任务组任意一个完成了的返回结果。
#### ExecutorServiceInstance.invokeAll
参数为集合，储存的是callable接口。也就是一系列任务组tasks，返回类型是装着callable的list，值为该系列任务组全部完成了的返回结果。
```
    public void testPool() throws ExecutionException, InterruptedException {
        ExecutorService pool = Executors.newFixedThreadPool(5);
        List<Callable<Integer>> tasks = new ArrayList<>();
        tasks.add(() -> atomicInteger.addAndGet(1));
...
执行器完成服务，可以看作ExecutorService的装饰类，可以在执行了一系列任务后，将future对象装入内置的阻塞队列中（按完成的先后顺序），需要用到future的时候去队列里面take就行了。
```
    public void testPool() throws ExecutionException, InterruptedException {
        ExecutorService pool = Executors.newFixedThreadPool(5);
        ExecutorCompletionService completionService = new ExecutorCompletionService(pool);
        for(int i=0;i<4;i++) {
            completionService.submit(() -> {TimeUnit.SECONDS.sleep(3);return atomicInteger.addAndGet(1);});
        }
        for(int i=0;i<4;i++) {
